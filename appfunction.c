...
void addaubaccount(byte *buf)
{
    ...
    else if(-2 == contains(aubaccountstr, (buf[3] + 1) * 2, ACCOUNTPW))
    {
        appsend[3] = 0x03;
    }
    else if(i >= 12)
...
void getautoscestatus(byte *buf)
{
    ...
    while(i < 21)
    {
        ...
        else
        {
            ...
            if(0 > (ascecontrol[i].interval - ((nowtime - autotri[i].lasttime) / 60)))
            {
                appsend[6] = 0;
                appsend[7] = 0;
            }
                    ...
                    if(manualsce[newtype] == 0)
                    {
                        ...
                        if(auto_logo == 0)
                        {
                            ...
                            if(appscene[4] == regcontrol[sub].bingkeymac[1])
                            {
                                ...
                                oldkey_logo = 1;
                                //执行状态
                                memcpy(appscene + 6, manualsceope_hex + 3, 5);
                            }
                            else
                            {
                                //执行状态
                        ...
                        case 0x31:
                            ...
                            */
                            ...
                            if(0 == memcmp(manualsceope_h, manualsceope_hex, 3))
                            {
                                pr_debug("case 0x01(2)\n");
                                //有改动
                                if(0 != memcmp(manualsceope_h, manualsceope_hex, 8))
                                {
                                    pr_debug("case 0x01(3)\n");
                                    //准备发送指令
                                    //id
                                    ...
                                    //执行状态
                                    memcpy(appscene + 6, manualsceope_h + 3, 5);
                                    //发送控制指令
                                    UART0_Send(uart_fd, appscene, appscene[1]);
                                    for(pr_i = 0; pr_i < 9; pr_i++)
                                    {
...
void networking(byte *buf)
{
    ...
    else if(buf[9] == 1)
    {
        ...
        if(hostinfo.slavemesh == 1)
        {
            ...
            sleep(3);
            system("sh /mnt/mtd/system_reset.sh");
            system("reboot");
        }
        else
        {
...
void delnodereg(byte *nodemac)
{
    ...
    while(i < 41)
    {
        ...
        while(type < regcontrol[i].tasklen)
        {
            ...
            {
                memcpy(tempreg + newtype, regcontrol[i].regtask + type, 8);
                newtype += 8;
                type += 8;
            }
        }
        ...
        while(type < mscecontrol[i].tasklen)
        {
            ...
            switch(mscecontrol[i].regtask[type])
            {
                ...
                case 0x31:
                    ...
                    {
                        memcpy(tempreg + newtype, mscecontrol[i].regtask + type, 10);
                        newtype += 10;
                        type += 10;
                    }
                    break;
...
void appmeshupgrade(byte *buf)
{
    ...
    sendsingle(client_fd, appsend, appsend[2]);
}
...
{
    int i = 2;
    byte appsend[24] = {0x96,0x00,0x16,0x00,0x0f};
    for(i = 2; i < 5; i++)
    {
        if(slaveident[i].assignid > 1)
        {
