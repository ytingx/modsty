...
//通过域名获取ip
int domaingetip(byte *serverip)
{
    FILE *fp = NULL;
    int i = 0;
    byte buf[512] = {0};
    byte cmd[128] = {"ping -c 1 server.modsty.com"};
    //byte cmd[128] = {"su - root ping -c 2 server.modsty.com"};
    ...
    sscanf(buf, "%*[^(](%[^)]s", serverip);
    pr_debug("serverip:%s\n", serverip);
    if(strlen(serverip) < 10)
    {
        return -1;
    }
    return 0;
...
void *sockclient_run(void *a)
{
    ...
    //char domainname[20] = "192.168.0.78";
    ...
    sinp.sin_port = htons(6001);
    sinp.sin_addr.s_addr = inet_addr("120.77.201.240");
    ...
    while(1)
    {
        if ((client_fd = socket(sinp.sin_family, SOCK_STREAM, 0)) < 0)
        {
        ...
        if (connect_retry(client_fd, (struct sockaddr*)&sinp, sizeof(sinp)) < 0)
        {
