...
//F8主从机透传指令
void slavepenetrate(int len, byte *buf)
{
    byte slavesocksend[1024] = {0xf8};

    //长度
    slavesocksend[1] = (((len + 4) >> 8) & 0xff);
    slavesocksend[2] = ((len + 4) & 0xff);
    //从机标识
    slavesocksend[3] = master;
    //拼接原始指令
    memcpy(slavesocksend + 4, buf, len);

    slaveupuartdata(len + 4, slavesocksend);
}

void slaveupload(int len, byte *buf)
{
    byte slavesocksend[1024] = {0xf7};
...
void *slaveserial_run(void *a)
{
    ...
    //打开8266串口，返回文件描述符
    uart_fd = UART0_Open(uart_fd, "/dev/ttyS1");
    do{
    ...
    //打开单片机串口，返回文件描述符
    single_fd = UART0_Open(single_fd, "/dev/ttyS0"); //打开8266串口，返回文件描述符
    do{
            ...
            for(i = 0; i < 2; i++)
            {
                ...
                while(1)
                {
                    ...
                    if(add != rcv_buf[typelen - 1])
                    {
                        ...
                        buf_t[strlen(buf_t)] = '\n';
                        InsertLine("/mnt/mtd/baduartlog", buf_t);
                        */
                        break;
                    }
                    if(temp > 0)
                    {
                        ...
                        buf_t[strlen(buf_t)] = '\n';
                        InsertLine("/mnt/mtd/uartuplog", buf_t);
                        */
                        ...
                        switch(rcv_buf[0])
                        {
                            ...
                            case 0x31://单片机上传
                                pr_debug("单片机上传\n");
                                slaveupload(((rcv_buf[1] << 8) + rcv_buf[2]), rcv_buf);
                                slavesinglechipup(rcv_buf);
                                break;
                            case 0x64://区域应答
                            case 0x67:
                            case 0x70:
