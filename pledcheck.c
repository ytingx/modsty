...
void checkbuttonled()
{
    ...
    while(i < 128)
    {
        ...
        if(smartcrlpanel[i].triid == 0)
        {
            pr_debug("按键LED检查完毕\n");
            break;
        }
        if(smartcrlpanel[i].type != 3)
        {
            i++;
            pr_debug("非按键关联区域\n");
            continue;
        }
        if(nodedev[smartcrlpanel[i].trimac[1]].nodeinfo[2] == 0x71)
        {
            i++;
            pr_debug("低功耗按键无需校验\n");
            continue;
        }
        id = smartcrlpanel[i].perftask[0];
        bmac = regcontrol[id].bingkeymac[1];
        bid = regcontrol[id].bingkeyid;
        
        if(regcontrol[id].tasklen == 0)
        {
            pr_debug("该按键关联的区域执行任务为0\n");
            i++;
            continue;
        }
        //区域有绑定功能键
        if((bid > 0) && ((bmac > 0) && (bmac < 0xff)))
        {
            //mac
            led[2] = smartcrlpanel[i].trimac[1];
            //type
            led[3] = 0x04;
            //node id
            led[4] = smartcrlpanel[i].triid;
            //led value
            led[5] = ((nodedev[bmac].nodeinfo[5] >> (bid - 1)) & 0x01);

            //检查led当前状态-区域绑定按键状态==关联按键状态
            if(((nodedev[led[2]].nodeinfo[5] >> (led[4] - 1)) & 0x01) == led[5])
            {
                pr_debug("按键关联led%d当前节点状态:%0x, 区域绑定功能键%d当前节点状态%0x\n", led[4], nodedev[led[2]].nodeinfo[5], bid, nodedev[bmac].nodeinfo[5]);
                i++;
                continue;
            }
            pr_debug("根据区域绑定按键校验led:%d\n", led[5]);
            Uart_Crc(uart_fd, led, led[1]);
            usleep(500000);
        }
        //无绑定按键
        else if((bid == 0) || (bmac == 0) || (bmac == 0xff))
        {
            //偏移
            h = 0;
            while(1)
            {
                //pr_debug("while-1\n");
                //拆分关联信息
                memcpy(rele_temp, regcontrol[id].regtask + h, 8);
                //检查完毕
                if(h == regcontrol[id].tasklen)
                {
                    //mac
                    led[2] = smartcrlpanel[i].trimac[1];
                    //type
                    ...
                    led[4] = smartcrlpanel[i].triid;
                    //检查led当前状态
                    if(0 == ((nodedev[led[2]].nodeinfo[5] >> (led[4] - 1)) & 0x01))
                    {
                        pr_debug("关-led%d当前节点状态:%0x\n", led[4], nodedev[led[2]].nodeinfo[5]);
                        break;
                    }
                    //led value
                    led[5] = 0x00;
                ...
                if(nodedev[rele_temp[2]].nodeinfo[2] == 0x31)
                {
                    if(0 < (nodedev[rele_temp[2]].nodeinfo[5] & 0x7f))
                    {
                        ...
                        led[4] = smartcrlpanel[i].triid;
                        //检查led当前状态
                        if(1 == ((nodedev[led[2]].nodeinfo[5] >> (led[4] - 1)) & 0x01))
                        {
                            pr_debug("开-led%d当前节点状态:%0x\n", led[4], nodedev[led[2]].nodeinfo[5]);
                            break;
                        }
                        //led value
                        led[5] = 0x01;
