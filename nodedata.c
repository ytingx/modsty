...
void createrestore()
{
    FILE *fp = NULL;
    int i = 0;
    byte restorefactory[2048] = {"#!/bin/sh\n\nrm -f /mnt/mtd/carmoxde\nrm -f /mnt/mtd/doormagnet\nrm -f /mnt/mtd/flooding\nrm -f /mnt/mtd/iractivity\nrm -f /mnt/mtd/rainsnow\nrm -f /mnt/mtd/gasinfo\nrm -f /mnt/mtd/windspeed\nrm -f /mnt/mtd/smokerelated\nrm -f /mnt/mtd/hoststatus\nrm -f /mnt/mtd/accountpw\nrm -f /mnt/mtd/masslainfo\nrm -f /mnt/mtd/meshpw\nrm -f /mnt/mtd/wifipw\nrm -f /mnt/mtd/initialpw\nrm -f /mnt/mtd/ipaddr.sh\nrm -f /mnt/mtd/nodeid\nrm -f /mnt/mtd/orinode\nrm -f /mnt/mtd/manualsce\nrm -f /mnt/mtd/autosce\nrm -f /mnt/mtd/autoenab\nrm -f /mnt/mtd/sensonenab\nrm -f /mnt/mtd/localass\nrm -f /mnt/mtd/regassoc\nrm -f /mnt/mtd/devname\nrm -f /mnt/mtd/acklog\nrm -f /mnt/mtd/led*\nrm -f /mnt/mtd/uart*\nrm -f /mnt/mtd/ir*\nrm -f /mnt/mtd/err*\nrm -f /mnt/mtd/secmonitor\nrm -f /mnt/mtd/smartlock\nrm -f /mnt/mtd/shake\nrm -f /mnt/mtd/smokerelated\nrm -f /mnt/mtd/lockrelated\nrm -f /mnt/mtd/gesture\nrm -f /mnt/mtd/smartcrlpanel\n"};
    byte systemreset[2048] = {"#!/bin/sh\n\nrm -f /mnt/mtd/carmoxde\nrm -f /mnt/mtd/doormagnet\nrm -f /mnt/mtd/flooding\nrm -f /mnt/mtd/iractivity\nrm -f /mnt/mtd/rainsnow\nrm -f /mnt/mtd/gasinfo\nrm -f /mnt/mtd/windspeed\nrm -f /mnt/mtd/smokerelated\nrm -f /mnt/mtd/nodeid\nrm -f /mnt/mtd/orinode\nrm -f /mnt/mtd/manualsce\nrm -f /mnt/mtd/autosce\nrm -f /mnt/mtd/autoenab\nrm -f /mnt/mtd/sensonenab\nrm -f /mnt/mtd/localass\nrm -f /mnt/mtd/regassoc\nrm -f /mnt/mtd/devname\nrm -f /mnt/mtd/acklog\nrm -f /mnt/mtd/led*\nrm -f /mnt/mtd/uart*\nrm -f /mnt/mtd/ir*\nrm -f /mnt/mtd/secmonitor\nrm -f /mnt/mtd/smartlock\nrm -f /mnt/mtd/shake\nrm -f /mnt/mtd/smokerelated\nrm -f /mnt/mtd/lockrelated\nrm -f /mnt/mtd/gesture\nrm -f /mnt/mtd/smartcrlpanel\n"};

    if((fp = fopen("/mnt/mtd/restorefactory.sh", "w")) == NULL)
    {
        pr_debug("/mnt/mtd/restorefactory.sh: %s\n", strerror(errno));
        exit(-1);
    }
    ...
    if((fp = fopen("/mnt/mtd/system_reset.sh", "w")) == NULL)
    {
        pr_debug("/mnt/mtd/system_reset.sh: %s\n", strerror(errno));
        exit(-1);
    }
...
int readaccount()
{
    ...
    if(-1 == access("/mnt/mtd/hoststatus", F_OK))
    {
        ...
        else
        {
            oriapsta[5] = '1';
            replace(HOSTSTATUS, oriapsta, 5);
            system("cp -f /mnt/mtd/ap/etc/config/* /etc/config/");
            if(hostinfo.mode == 1)
            {
                HexToStr(wifiapssidstr, hostinfo.hostsn, 6);
            }
            else
            {
                HexToStr(wifiapssidstr, hostinfo.slavesn, 6);
            }
            memcpy(wifiapssid + 31, wifiapssidstr + 8, 4);
            pr_debug("AP MODE:%s\n", wifiapssid);
            system(wifiapssid);
            system("uci commit");
        }
        system("cp -f /mnt/mtd/samb/smb.conf* /etc/samba/");
        system("cp -f /mnt/mtd/samb/smbpasswd /etc/samba/");
        system("cp -f /mnt/mtd/samb/samba /etc/config/");
    ...
    if(-1 == access("/mnt/mtd/initialpw", F_OK))
    {
        pr_debug("创建initialpw\n");
        replace(INITIALPW, oripw, 12);
    }
    if(-1 == access("/mnt/mtd/regassoc", F_OK))
    {
        pr_debug("创建regassoc\n");
...
void searchnode(int slave, byte *node)
{
    ...
    {
        pr_debug("hostinfo.mesh:%d\n", hostinfo.mesh);
        if((hostinfo.mesh == 1) || (hostinfo.addnode == node[2]))
        {
            //hostinfo.mesh = 1;
...
void nodeupdate(int slave, byte *node)
{
    ...
    switch(node[3])
    {
        ...
        case 0x0c:
            ...
            //检查负载&传感器更新
            autorefresh[1] = node[2];
            autorefresh[4] = node[5];
            checkstatuschange(0x0c, autorefresh, statuschange);
            stateapp[8] = statuschange[0];
            stateapp[9] = statuschange[1];
            stateapp[2] = 0x0a;
...
void checkstatuschange(int type, byte *status, byte *change)
{
    ...
    {
        //负载/led
        if(status[3] != nodedev[status[1]].nodeinfo[5])
        {
            nodestatus = nodedev[status[1]].nodeinfo[5];
            while(i <= 0x80)
            {
                if((status[3] & i) != (nodestatus & i))
                {
                    change[0] |= i;
                }
                i *= 2;
                //pr_debug("change[0]:%0x\n", change[0]);
                //pr_debug("i:%d\n", i);
            }
        }
        //传感器
        if(status[4] != nodedev[status[1]].nodeinfo[6])
        {
            i = 1;
            nodestatus = nodedev[status[1]].nodeinfo[6];
            while(i <= 0x08)
            {
                if((status[4] & i) != (nodestatus & i))
                {
                    change[1] |= i;
                }
                i *= 2;
                //pr_debug("change[1]:%0x\n", change[1]);
                //pr_debug("i:%d\n", i);
            }
            if((status[4] & 0xf0) != (nodestatus & 0xf0))
            {
                change[1] |= 0x10;
            }
        }
        //温度
        if(status[5] != nodedev[status[1]].nodeinfo[7])
        {
            change[1] |= 0x20;
        }
    }
    ...
    {
        //使能
        if(status[4] != nodedev[status[1]].nodeinfo[8])
        {
            nodestatus = nodedev[status[1]].nodeinfo[8];
            while(i <= 0x40)
            {
                if((status[4] & i) != (nodestatus & i))
                {
...
void statefeedback(int slave, byte *temnode)
{
    ...
    while(node[offset] != 0)
    {
        ...
        memset(autorefresh, 0, sizeof(autorefresh));
        ...
        else
        {
            ...
            if((nodedev[node[offset]].nodeinfo[2] > 0) && (nodedev[node[offset]].nodeinfo[2] < 0x0f))
            {
                ...
                else
                {
                    ...
                    if((nodedev[node[offset]].nodeinfo[5] & 0xf0) != (node[offset + 2] & 0xf0))
                    {
                        ...
                        //检查负载&传感器更新
                        checkstatuschange(0x01, autorefresh, statuschange);
                        stateapp[10] = statuschange[0];
                        stateapp[11] = statuschange[1];
                    ...
                    else if(nodedev[node[offset]].nodeinfo[6] != node[offset + 3])
                    {
                        ...
                        //检查负载&传感器更新
                        checkstatuschange(0x13, autorefresh, statuschange);
                        stateapp[10] = statuschange[0];
                        stateapp[11] = statuschange[1];
            ...
            //智能控制面板
            else if((nodedev[node[offset]].nodeinfo[2] > 0x71) && (nodedev[node[offset]].nodeinfo[2] < 0x80))
            {
                //离线
                if(node[offset + 1] == 0x00)
                {
                ...
                else
                {
                    ...
                    //led状态更新
                    if(nodedev[node[offset]].nodeinfo[5] != node[offset + 2])
                    {
                        //上报app
                        //mac
                        stateapp[4] = node[offset];
            ...
            else
            {
                ...
                else
                {
                    ...
                    //len
                    stateapp[2] = 0x0a;
                    if((nodedev[node[offset]].nodeinfo[2] > 0x20) && (nodedev[node[offset]].nodeinfo[2] < 0x30))
                    { 
                        //更新自动场景条件
                        autorefresh[0] = slave;
                        memcpy(autorefresh + 1, node + offset, 5);
                    ...
                    if((nodedev[node[offset]].nodeinfo[2] > 0x20) && (nodedev[node[offset]].nodeinfo[2] < 0x40))
                    {
                        pr_debug("触发led校验\n");
                        pthread_mutex_lock(&fifo->lock);
                        g_led = 1;
...
//智能控制面板执行区域
void smartctlpanelassarea(int sub, byte *key)
{
    byte uartsend[24] = {0x65,0x0c};
    byte appsend[12] = {0xd3,0x00,0x04,0x00};
    //id
    uartsend[2] = smartcrlpanel[sub].perftask[0];
    if((uartsend[2] > 0) && (regcontrol[uartsend[2]].tasklen > 0))
    {
        //mac
        if((uartsend[2] > 0) && (uartsend[2] < 5))
        {
        ...
        uartsend[4] = regcontrol[uartsend[2]].bingkeyid;
        //控制类型
        uartsend[5] = smartcrlpanel[sub].perftask[1];
        memcpy(uartsend + 5, smartcrlpanel[sub].perftask + 1, 4);

        if(uartsend[5] == 0x00)
        {
            //0x71低功耗设备单独处理
            if(nodedev[smartcrlpanel[sub].trimac[1]].nodeinfo[2] == 0x71)
            {
                pr_debug("0x71:%d\n", uartsend[2]);
                uartsend[6] = smartcrlpanel[sub].perftask[2];
            }
            else
            {
                if(0x01 == key[3])
    ...
    for(i = 0; i < 128; i++)
    {
        ...
        else if(0 == memcmp(smartcrlpanel[i].trimac, key, 2))
        {
            if(smartcrlpanel[i].triid == key[2])
            {
                ...
                //区域
                else if(smartcrlpanel[i].type == 0x03)
                {
                    //智能控制面板执行区域
                    smartctlpanelassarea(i, key);
                    return;
                }
                //手动场景
                else if(smartcrlpanel[i].type == 0x04)
                {
                    //id
...
void touchfunction(int slave, byte *touch, byte nodetype)
{
    ...
    else if((touch[4] & 0x80) != 0)
    {
        ...
        if((nodetype > 0) && (nodetype < 0x0f))
        {
            ...
            else if((nodetype == 0x04) && (nodedev[touch[2]].nodeinfo[4] < 4))
            {
                if(nodedev[touch[2]].nodeinfo[4] == 3)
                {
                    if(touchkey[2] > 7)
                    {
                        //智能控制面板长按
                ...
                else if(nodedev[touch[2]].nodeinfo[4] == 2)
                {
                    if(touchkey[2] > 6)
                    {
                        //智能控制面板长按
                ...
                else if(nodedev[touch[2]].nodeinfo[4] == 1)
                {
                    if(touchkey[2] > 5)
                    {
                        //智能控制面板长按
        ...
        else if((nodetype > 0x71) && (nodetype < 0x75))
        {
            //开关-可切换负载/按键
            //功能键长按
            if((touchkey[2] > 0) && (touchkey[2] < 5))
    ...
    else if((touch[4] & 0x80) == 0)
    {
        ...
        if((nodetype > 0) && (nodetype < 0x0f))
        {
            ...
            //本地键短按
            else if((touchkey[2] > 4) && (touchkey[2] < 9))
            {
                if((nodetype == 0x04) && (nodedev[touch[2]].nodeinfo[4] < 4))
                {
                    if(nodedev[touch[2]].nodeinfo[4] == 3)
                    {
                        if(touchkey[2] > 7)
...
void restorefactory()
{
    ...
    sleep(5);
    pr_debug("恢复出厂等待5秒\n");
    system("sh /mnt/mtd/restorefactory.sh");
    system("reboot");
}
...
void meshupdataok()
{
    byte appsend[8] = {0xa5,0x00,0x04,0x03};
...
void smartctrpanelanswer(int slave, byte *buf)
{
    ...
    if(buf[11] == 0)
    {
        ...
        {
            //清除所有按键任务
            system("rm -f /mnt/mtd/smartcrlpanel");
            //清除内存中的按键信息
            memset(&smartcrlpanel, 0, sizeof(smartcrlpanel));
        }
        else
